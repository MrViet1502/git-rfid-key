...
void storeCard(byte *uid) // function store card
{
    buzzerBeep(1, 500);
    // loadCardsFromEEPROM();
    if (cardCount == 0)
...
bool readNFC()
{
    if (nfc.tagPresent())
    {
        ...
        // for (int i = 0; i < 4; i++)
        // {
        //   if (registeredCards[i].uidLength == 0) // Kiểm tra nếu vị trí chưa có thẻ nào
        //   {
        //     memcpy(registeredCards[i].uid, uid, uidLength);
        //     registeredCards[i].uidLength = uidLength;
        //     Serial.print("Stored UID in slot ");
        //     Serial.println(i);
        //     cardCount++;
        //     break;
        //   }
